...
void TrafficLight::cycleThroughPhases()
{
    ...
    int interval = getRandom();
    auto lastTimestamp = std::chrono::system_clock::now();

    while(true) {
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
        auto now = std::chrono::system_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(now - lastTimestamp).count();
        if (duration >= interval) {
            lastTimestamp = now;
            interval = getRandom();
            auto phase = getCurrentPhase();
            auto nextPhase = phase == TrafficLightPhase::red ? TrafficLightPhase::green : TrafficLightPhase::red;
